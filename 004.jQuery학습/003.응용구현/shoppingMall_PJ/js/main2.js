...
$(()=>{
    ...
    [ 세로방향 배너 요구사항 ]
        ...
        1. 아래쪽버튼 클릭시 배너는 아랫쪽에서 윗쪽방향으로
        이동하여 다음 슬라이드가 보임

        2. 윗쪽버튼 클릭시 배너는 윗쪽에서 아랫쪽 방향으로
        이동하여 이전 슬라이드가 보임
    ...
    ************************************************/
        ...
        $(".abtn").click(function(){
            ...
            // 2. 버튼별 분기하여 기능구현
            if(isR){  //아랫쪽버튼  //{CSS설정},시간,이징,이동 후 실행함수
                slide.animate({top:"-100%"},aniT,aniE,function(){ //이동 후 실행함수
                    // append(요소)
                    // -> 자식요소로 맨뒤추가(이동)
        ...
        function autoSlide(){
            ...
            autoI = setInterval(()=>{
                slide.animate({top:"-100%"},aniT,aniE,function(){ 
                    $(this).append($("li",this).first())
                    .css({top:"0"});
                }); /////// animate ///////////
